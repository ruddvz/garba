...
5. If compatible BPM metadata exists and the browser permits playback-rate adjustment, the incoming track can start with a small tempo correction and settle back to its original speed during the transition.
...
The normal path uses the browser's media-element volume controls for the equal-power blend.

Some browsers do not provide reliable programmable `HTMLMediaElement.volume`. On those browsers Garba loads `assets/runtime/automix-web-audio.js`, unlocks a Web Audio context from a trusted playback gesture, routes the two media elements through `GainNode`s, and schedules the same equal-power curve in the audio graph.
...
If the incoming file cannot preload, the second deck cannot start, or the browser cannot provide safe gain control, the transition is abandoned and the existing single-track playback path remains in control.
