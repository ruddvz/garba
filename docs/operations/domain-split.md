...
During this migration, `.github/workflows/pages.yml` keeps those source validations intact and rewrites crawlable URLs only after validation, catalogue generation and social-preview injection. The uploaded Pages artifact therefore uses `https://live.playgarba.com` for:

- the player canonical URL;
- Open Graph and Twitter URLs, including the universal social card;
