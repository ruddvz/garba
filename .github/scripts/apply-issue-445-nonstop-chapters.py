...
source = replace_once(
    ...
    "state chapter fields",
)

format_anchor = "  function recordingPresentation(set) {"
chapter_helpers = """  function elapsedSecondsFromText(value) {
  ...
  function verifiedChaptersForSet(set) {
    ...
    segments.forEach((segment, sourceIndex) => {
      ...
      if (!title || !Number.isFinite(startSeconds) || startSeconds < 0 || startSeconds <= previousStart) return;
      chapters.push({ title, startSeconds, sourceIndex });
      previousStart = startSeconds;
    });
    return chapters;
  }

  function currentChapterIndex(chapters, elapsedSeconds) {
...
""" + format_anchor
source = replace_once(source, format_anchor, chapter_helpers, "chapter helpers")
...
chapter_css = """      .nonstop-chapters{padding:0 24px 12px;border-bottom:1px solid rgba(246,236,215,.08)}
      ...
      .nonstop-chapter{flex:0 0 min(250px,70vw);min-height:54px;display:grid;grid-template-columns:30px minmax(0,1fr) auto;gap:9px;align-items:center;text-align:left;border:1px solid rgba(246,236,215,.09);border-radius:15px;background:rgba(255,255,255,.025);color:inherit;padding:9px 11px;cursor:pointer}
...
""" + list_css
source = replace_once(source, list_css, chapter_css, "chapter styles")
...
set_background_anchor = "  function setBackgroundInert(inert) {"
chapter_runtime = """  function seekNonstopChapter(chapter, chapterIndex) {
  ...
  function syncChapterState(elapsedOverride = null) {
    ...
    const nextIndex = currentChapterIndex(chapters, observed);
    if (state.currentChapterIndex === nextIndex && section.querySelector(`[data-nonstop-chapter-index="${nextIndex}"][aria-current="true"]`)) return;
  ...
  function renderChapterNavigation() {
    ...
    chapters.forEach((chapter, chapterIndex) => {
      ...
      button.setAttribute('aria-label', `Jump to chapter ${chapterIndex + 1}, ${chapter.title}, ${time}`);
...
""" + set_background_anchor
source = replace_once(source, set_background_anchor, chapter_runtime, "chapter runtime")

source = replace_once(
    source,
    "      state.activeSet = set;\n      state.activeTrack = track;\n      setMetadata(set);",
    "      state.activeSet = set;\n      state.activeTrack = track;\n      state.currentChapterIndex = -1;\n      setMetadata(set);",
    "chapter reset on set start",
)
...
chapter_guard_anchor = "const trackForSetStart = source.indexOf('function trackForSet(set)');"
chapter_guard = """const chapterBuilderStart = source.indexOf('function verifiedChaptersForSet(set)');
...
""" + chapter_guard_anchor
validator = replace_once(validator, chapter_guard_anchor, chapter_guard, "validator chapter guards")

log_anchor = "console.log('✓ Nonstop queue accessibility state is reasserted when ordinary queue metadata changes');"
validator = replace_once(
    validator,
    log_anchor,
    log_anchor + "\nconsole.log('✓ verified Nonstop chapters render only from published timestamps, track elapsed playback, and jump through the existing seek contract');",
