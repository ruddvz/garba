...
function syncMediaArtwork() {
  ...
  } catch { /* some webviews reject artwork */ }
}
...
function categoryTerms(song) {
  ...
  for (const id of ids) {
    ...
    if (!category) terms.push(id);
    else terms.push(category.id, category.label, ...(category.aliases || []));
  }
  return terms;
}
...
function renderEnhancedSearch() {
  ...
  if (nativeRows.length || songList.querySelector('.enhanced-search-row')) return;
...
async function loadNextContext() {
  ...
  } catch { /* core catalogue remains authoritative */ }
  finally { scheduleEnhancedSearch(); }
}
