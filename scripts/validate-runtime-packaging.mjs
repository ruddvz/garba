...
for (const file of ['assets/runtime/garba-circle.js', 'assets/runtime/garba-circle-controller.js', 'assets/runtime/qr-code.js', 'assets/runtime/live-sync.js']) {
  if (!sw.includes(`'./${file}'`)) fail(`PWA core shell does not cache ${file}`);
  if (!sw.includes(`'/${file}'`)) fail(`PWA fresh-runtime list does not include ${file}`);
}
...
if (!appSource.includes("from './assets/runtime/live-sync.js'")) fail('app.js must load Live Radio sync from assets/runtime');
...
console.log('✓ Garba Circle and Live Radio sync modules ship with assets/runtime and are precached network-first');
