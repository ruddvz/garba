...
const songs = await readJson('data/ui-demo-songs.json');
...
for (const genre of genres) {
  ...
  if (!/\.webp$/i.test(genre.background || '')) fail(`Production background should be WebP: ${genre.id}`);
}
...
for (const song of songs) {
  ...
  if (!Number.isFinite(song.durationSeconds) || song.durationSeconds < 0) fail(`Invalid durationSeconds on ${song.id}`);
...
if (!Array.isArray(manifest.icons) || manifest.icons.length < 3) fail('Manifest needs standard and maskable icons');
