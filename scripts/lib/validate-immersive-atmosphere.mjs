...
if (manifest.version !== '1.2.0' && manifest.version !== '1.1.0') fail('Atmosphere source manifest version must be valid');
...
for (const source of enabledSources) {
  if (!['public-domain', 'CC-BY-SA-3.0', 'CC-BY-SA-4.0'].includes(source.license)) fail(`Enabled Atmosphere source ${source.id} must be approved license`);
  if (source.containsMusic !== false) fail(`Enabled Atmosphere source ${source.id} must explicitly contain no music`);
  if (!/^(https:\/\/|assets\/)/.test(source.audioUrl || '')) fail(`Enabled Atmosphere source ${source.id} must use valid asset path or HTTPS`);
}
