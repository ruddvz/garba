...
if (/\bSoundstage\b|\bStereo Expand\b/.test(runtime)) {
...
for (const source of enabledSources) {
  if (!['public-domain', 'CC-BY-SA-3.0', 'CC-BY-SA-4.0'].includes(source.license)) fail(`Enabled Atmosphere source ${source.id} must use an approved open license`);
  if (source.containsMusic !== false) fail(`Enabled Atmosphere source ${source.id} must explicitly contain no music`);
  const url = source.remoteUrl || source.audioUrl || '';
  if (!/^https:\/\//.test(url) && !/^assets\//.test(url)) fail(`Enabled Atmosphere source ${source.id} must use HTTPS or local assets`);
}
