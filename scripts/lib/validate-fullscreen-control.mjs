...
import { buildFullscreenControlRuntime } from './build-fullscreen-control-runtime.mjs';
...
const generatedRuntime = await buildFullscreenControlRuntime();
...
assert.equal(runtimeSource, generatedRuntime, 'browser runtime must be generated from the canonical fullscreen policy/controller/fact-source contracts');
assert.match(runtimeSource, /Generated by scripts\/lib\/build-fullscreen-control-runtime\.mjs/, 'generated runtime must identify its canonical builder');
assert.match(runtimeSource, /createFullscreenController/, 'generated runtime must consume the canonical fullscreen controller');
assert.match(runtimeSource, /readFullscreenFacts/, 'generated runtime must consume the canonical fullscreen fact source');
assert.equal(packageJson.scripts['fullscreen:build'], 'node scripts/lib/build-fullscreen-control-runtime.mjs', 'deterministic fullscreen build script must be registered');
...
assert.match(packageJson.scripts['check:modules'], /node --check scripts\/lib\/build-fullscreen-control-runtime\.mjs/, 'module syntax gate must cover fullscreen runtime builder');
...
assert.doesNotMatch(runtimeSource, /\buserAgent\b|\bplatform\b\s*[=!]=|requestFullscreen\(\).*setTimeout/i, 'fullscreen availability must not depend on UA sniffing or delayed auto-entry');
...
{
  const scenario = createScenario({ manifestFullscreen: true, supported: true });
  assert.equal(scenario.button.hidden, false, 'canonical fact source must treat fullscreen display mode as app-like');
  assert.equal(scenario.calls.request, 0, 'app-like display facts must never auto-enter the Fullscreen API');
}

{
  ...
  assert.equal(JSON.stringify(scenario.app.playerState), sentinel, 'entering fullscreen must not mutate playback state');
  ...
  assert.equal(JSON.stringify(scenario.app.playerState), sentinel, 'external fullscreen exit must not mutate playback state');
}
...
console.log('✓ fullscreen browser runtime is generated from the canonical policy/controller/fact-source contracts');
